...
//		this.shapeVector.add(this.currentShape);
	...
	public void selectCopy(int x, int y){
		for(GShape shape : this.shapeVector){
			...
			if(eAnchor != null){
				...
				ox = shape.getShape().getBounds().x;
				oy = shape.getShape().getBounds().y;
				w = shape.getShape().getBounds().width+1;
				h = shape.getShape().getBounds().height+1;
				copyShape = shape;
			}
		}
	...
	private void drawCopy(int x, int y) {
		int dx, dy;
		...
		copyShape.drawCopy(ox, oy, w, h, dx, dy);
